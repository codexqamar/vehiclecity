...
const CARDS: Record<
  string,
  Array<{ reg: string; car: string; who: string; svc: string; eta: string }>
> = {
  queue: [
  ...
  parts: [{ reg: "EV70 BYD", car: "Tesla Model 3", who: "Sam", svc: "12V battery", eta: "16:20" }],
  ready: [
    {
      reg: "RV68 OMG",
            ...
            <div key={c.id} className="w-72 shrink-0">
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                  <span className={cn("rounded-full px-2 py-0.5 text-[11px] font-medium", c.tone)}>
                    {c.title}
                  </span>
              ...
              <div className="mt-2 space-y-2">
                {CARDS[c.id].map((card) => (
                  <div
                    key={card.reg}
                    className="cursor-grab rounded-lg border border-border bg-card p-3 shadow-soft transition hover:border-foreground/15"
                  >
